...
A evolução dos sistemas web para arquiteturas modernas, como microsserviços e aplicações baseadas em nuvem, transformou significativamente a superfície de ataque. Com a fragmentação de aplicações monolíticas em serviços web distribuídos, a comunicação entre processos e a exposição de APIs tornaram-se centrais para o funcionamento e, consequentemente, para a segurança dessas aplicações.

\subsubsection{
    APIs REST — OWASP API Top 10, BOLA, \textit{rate limiting}
}\label{sec:apis_rest}

As APIs REST são predominantes em aplicações modernas e concentram riscos relevantes. Segundo o OWASP API Top 10 (2023), a vulnerabilidade de maior destaque é \textit{Broken Object Level Authorization} (BOLA), que ocorre quando o backend expõe identificadores de objetos sem validar se o usuário autenticado possui permissão para acessá-los. Esse cenário permite leitura, alteração ou exclusão indevida de dados de terceiros.
